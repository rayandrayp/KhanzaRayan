...
package simrskhanza;
import bridging.ApiBPJSAplicare;
...
public class DlgKamarInap extends javax.swing.JDialog {
    ...
    private String now=dateFormat.format(date),kmr="",key="",tglmasuk,jammasuk,kd_pj,
            hariawal="",pilihancetak="",aktifkan_hapus_data_salah="",link="",user="",noSEP="";
    ...
    ArrayList<String[]> referensiKamar = new ArrayList<String[]>();
    ...
    private void initComponents() {
        ...
        DTPCari3.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "08-02-2022" }));
        ...
        DTPCari4.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "08-02-2022" }));
    ...
    private void myInitComponents() {
        ...
        internalFrame10.add(BtnCloseIn10);
        BtnCloseIn10.setBounds(405, 92, 100, 30);
        ...
        internalFrame10.add(BtnSimpan10);
        BtnSimpan10.setBounds(300, 92, 100, 30);

        jLabel41.setText("Tgl.Pulang :");
        jLabel41.setName("jLabel41"); // NOI18N
        internalFrame10.add(jLabel41);
        jLabel41.setBounds(6, 32, 70, 23);
        ...
        internalFrame10.add(TanggalPulang);
...
//            }
//        });
        internalFrame10.add(StatusPulang);
        ...
        internalFrame10.add(TanggalKematian);
        ...
        internalFrame10.add(NoLPManual);
...
//                    updateWS(tbKamIn.getValueAt(tbKamIn.getSelectedRow(),19).toString(), kdkamarpindah.getText());
                    tampil();
                    WindowPindahKamar.dispose();
                    break;
            }            
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(tbKamIn.getSelectedRow()>-1){
                if(norawat.getText().trim().equals("")){
                    ...
                    Sequel.mengedit("kamar","kd_kamar='"+tbKamIn.getValueAt(tbKamIn.getSelectedRow(),19).toString()+"'","status='KOSONG'");  
//                    updateWS(tbKamIn.getValueAt(tbKamIn.getSelectedRow(),19).toString(), "");
                    tampil();
                }        
            }
        }
...
//        }
                ...
                if(tbKamIn.getValueAt(tbKamIn.getSelectedRow(),0).toString().equals("")){
                    try {
                        ...
                        try {
                            ...
                            rs2=psanak.executeQuery();
                            if(rs2.next()){
                                ps=koneksi.prepareStatement("select * from bridging_sep where no_rawat=? ORDER BY no_sep DESC ");
                                try {
    ...
    */
    ...
    private void updateWS(String kamar1, String kamar2){
        String kodekelas, koderuang, namaruang, kelas;
        kodekelas = koderuang = namaruang = kelas = "";
        int tersedia, tersediapria, tersediawanita, tersediapriawanita, kapasitas;
        tersedia = tersediapria = tersediawanita = tersediapriawanita = kapasitas = 0;
        String[] arrOfStr = kamar1.split(".", 2);
        String[] arrOfStr2 = kamar2.split(".", 2);
        
        try{
            ps=koneksi.prepareStatement(
                "SELECT k.`status` , COUNT(k.`status`) AS jml, k.kelas , b.nm_bangsal, k.kd_bangsal  " +
                "FROM kamar k " +
                "INNER JOIN bangsal b ON b.kd_bangsal = k.kd_bangsal " +
                "WHERE k.kd_kamar LIKE '"+arrOfStr[0]+"%' AND k.statusdata = '1' GROUP BY k.`status`, k.kelas");
            try {
                rs=ps.executeQuery();
                getReferensiKamar();
                while(rs.next()){
                    if (rs.getString("status") == "KOSONG"){
                        tersediapriawanita = rs.getInt("jml");
                        tersedia = rs.getInt("jml");
                    }
                    kapasitas += rs.getInt("jml");
                    namaruang = rs.getString("nm_bangsal");
                    kelas = rs.getString("kelas");
                    koderuang = rs.getString("kd_bangsal");
                }
                //get kode kelas from WS referensi kelas
                for (int i = 0; i<referensiKamar.size(); i++){
                    if (referensiKamar.get(i)[1] == kelas){
                        kodekelas = referensiKamar.get(i)[0];
                    }
                }
        ...
        requestJson ="{\"kodekelas\":\""+kodekelas+"\", "+
                    ...
                    "}";
        sendDataWS("/rest/bed/update/", requestJson);
        
        if (kamar2 != ""){
            tersedia = tersediapria = tersediawanita = tersediapriawanita = kapasitas = 0;
            try{
                ps=koneksi.prepareStatement(
                    "SELECT k.`status` , COUNT(k.`status`) AS jml, k.kelas , b.nm_bangsal " +
                    "FROM kamar k " +
                    "INNER JOIN bangsal b ON b.kd_bangsal = k.kd_bangsal " +
                    "WHERE k.kd_kamar LIKE '"+arrOfStr2[0]+"%' AND k.statusdata = '1' GROUP BY k.`status`, k.kelas");
                try {
                    rs=ps.executeQuery();
                    while(rs.next()){
                        if (rs.getString("status") == "KOSONG"){
    ...
    private void sendDataWS(String path, String request){
        try {
            ...
            root = mapper.readTree(api.getRest().exchange(URL+path+kodeppk, HttpMethod.POST, requestEntity, String.class).getBody());
            ...
            if(nameNode.path("message").asText().equals("OK")){
                ...
                if(response.path("list").isArray()){
                    ...
                    for(JsonNode list:response.path("list")){
                      referensiKamar.add(new String[] {list.path("kodekelas").asText(), list.path("namakelas").asText()});
                    }
                }
            }else {
                JOptionPane.showMessageDialog(null,nameNode.path("message").asText());                
    ...
    private void StatusPulangItemStateChanged(java.awt.event.ItemEvent evt) {                                              
    ...
    private void ppPulangBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {                                                 
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR)); 
...
//            }
    ...
    private void BtnCloseIn10ActionPerformed(java.awt.event.ActionEvent evt) {                                            
        WindowUpdatePulang.dispose();
    }                     
    
    private void BtnSimpan10ActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            ps=koneksi.prepareStatement("select * from bridging_sep where no_rawat=? ORDER BY no_sep DESC ");
            try {
                ps.setString(1,TNoRwCari.getText());
            ...
            if(nameNode.path("code").asText().equals("200")){
                ...
                if (reply == JOptionPane.YES_OPTION) {
                    ...
                    if(Sequel.cariRegistrasi(TNoRMCari.getText())>0){
                        JOptionPane.showMessageDialog(rootPane,"Data billing sudah terverifikasi..!!");
                    }else{ 
                        ...
                        kamarinap.emptTeks();
                        kamarinap.isCek();
                        kamarinap.setNoRm(TNoRMCari.getText(),TNoRM.getText(),TPasien.getText()); 
