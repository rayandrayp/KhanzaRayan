...
public final class AplicareKetersediaanKamar extends javax.swing.JDialog {
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(KdKelas.getText().trim().equals("")||NmKelas.getText().trim().equals("")){
            ...
            try {
                System.out.println("begin save TT");
                ...
                if(nameNode.path("message").asText().equals("Data berhasil disimpan.")){
                    if(Sequel.menyimpantf("aplicare_ketersediaan_kamar","?,?,?,?,?,?,?,?","Data",8,new String[]{
                            KdKelas.getText(),KdKamar.getText(),NmKelas.getText().toString(),Kapasitas.getText(),
...
//            System.out.println(KdKamar.getText()+" "+KdKelas.getText());
            if(KdKamar.getText().trim().equals("")||KdKelas.getText().trim().equals("")){
                Valid.textKosong(KdKamar,"Silahkan pilih terlebih dahulu.");
            } else {
//            if(tbJnsPerawatan.getValueAt(i,0).toString().equals("true")){
                try {
                    System.out.println("begin delete TT");
                    headers = new HttpHeaders();
                    headers.setContentType(MediaType.APPLICATION_JSON);
                    headers.add("X-Cons-ID",CONSIDAPIAPLICARE);
                    utc=String.valueOf(api.GetUTCdatetimeAsString());
...
//        }  
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(KdKelas.getText().trim().equals("")){
            ...
            try {     
                System.out.println("begin update TT");
                headers = new HttpHeaders();
                headers.setContentType(MediaType.APPLICATION_JSON);
                headers.add("X-Cons-ID",CONSIDAPIAPLICARE);
                utc=String.valueOf(api.GetUTCdatetimeAsString());
...
//        }
	    ...
	    requestEntity = new HttpEntity(headers);
            root = mapper.readTree(api.getRest().exchange(URL+"/rest/bed/read/"+kodeppk+"/1/100", HttpMethod.GET, requestEntity, String.class).getBody());
            response = root.path("response");
            //System.out.println("code : "+nameNode.path("code").asText());
            //System.out.println("message : "+nameNode.path("message").asText());
//            System.out.println("X-Cons-ID " + koneksiDB.CONSIDAPIAPLICARE());
//            System.out.println("user_key " + koneksiDB.USERKEYAPIAPLICARE());
            
            Valid.tabelKosong(tabMode);
            //response = mapper.readTree(api.Decrypt(root.path("response").asText(),utc));
            if(response.path("list").isArray()){
                for(JsonNode list:response.path("list")){
//                    System.out.println(list.path("rownumber").asText()+" "+list.path("kodekelas").asText()+" "+list.path("namakelas").asText());
                    if(list.path("kodekelas").asText().toLowerCase().contains(TCari.getText().toLowerCase())||
                        list.path("koderuang").asText().toLowerCase().contains(TCari.getText().toLowerCase())||
                        list.path("namaruang").asText().toLowerCase().contains(TCari.getText().toLowerCase())||
                        list.path("namakelas").asText().toLowerCase().contains(TCari.getText().toLowerCase())){
//                        System.out.println(list.path("rownumber").asText()+" "+list.path("kodekelas").asText()+" "+list.path("namakelas").asText());
//                        System.out.println(list.path("rownumber").asText()+" "+list.path("koderuang").asText()+" "+list.path("namaruang").asText());
//                        System.out.println(list.path("rownumber").asText()+" "+list.path("tersedia").asText()+" "+list.path("kapasitas").asText());
                        tabMode.addRow(new Object[]{
                            list.path("rownumber").asText(),list.path("kodekelas").asText(),list.path("koderuang").asText(),
                            list.path("namaruang").asText(),list.path("namakelas").asText(),list.path("kapasitas").asText(),
                            list.path("tersedia").asText(),list.path("tersediapriawanita").asText(),list.path("tersediapria").asText(),
                            list.path("tersediawanita").asText(),list.path("lastupdate").asText()
                        });
                    }
                }
            }  
    ...
    private void getData() {
       if(tbJnsPerawatan.getSelectedRow()!= -1){
           ...
           NmKelas.setText(tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(),4).toString());
